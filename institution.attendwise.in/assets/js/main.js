...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Select only inputs with the specific class
    const inputs = document.querySelectorAll('.msc-file-upload');
    ...
    function initializeFileUpload(input) {
        ...
        wrapper.className = 'msc-upload-wrapper';

        // 3. Create the Drop Zone HTML
        const dropZone = document.createElement('div');
        dropZone.className = 'msc-drop-zone';
        dropZone.innerHTML = `
            <svg class="msc-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"/></svg>
            <div class="msc-prompt">Drag & Drop or <span>Browse</span></div>
            <div class="msc-file-info"></div>
        `;

        // Insert the wrapper after the input, then move input inside (optional)
        // Here we keep input outside but visually replace it
        input.parentNode.insertBefore(wrapper, input.nextSibling);
        wrapper.appendChild(dropZone);

        const fileInfo = dropZone.querySelector('.msc-file-info');

        // --- Event Listeners ---

        // 1. Handle Click (Trigger original input)
        dropZone.addEventListener('click', () => input.click());

        // 2. Handle File Selection (Standard Input Change)
        input.addEventListener('change', () => {
            updateFileInfo(input.files, fileInfo);
        });
        ...
        ['dragenter', 'dragover'].forEach(eventName => {
            dropZone.addEventListener(eventName, () => dropZone.classList.add('msc-drag-over'), false);
        });

        ['dragleave', 'drop'].forEach(eventName => {
            dropZone.addEventListener(eventName, () => dropZone.classList.remove('msc-drag-over'), false);
        });
        ...
        dropZone.addEventListener('drop', (e) => {
            ...
            // Note: This requires modern browser support
            input.files = files;

            // Update UI
            updateFileInfo(files, fileInfo);
        });
    }

    // Helper: Update UI text
    function updateFileInfo(files, displayElement) {
        if (files.length > 0) {
