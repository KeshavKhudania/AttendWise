...
class AdminUserController extends Controller
{
    ...
    function formView(Request $req){
        ...
        if ($req->segment(3)) {
            ...
            $data = [
                "title"=>"Edit User",
                "type"=>"EDIT",
                "action"=>route("institution.admin.user.update", ["id"=>$req->segment(3)]),
                "permissions"=>unserialize(AdminGroup::find(get_logged_in_user()->admin_group_id)->permissions),
                ...
                "fields"=>[
                    ...
                    "group_id"=>$details->admin_group_id,
            ...
            $data = [
                "title"=>"Create User",
                "type"=>"ADD",
                "action"=>route("institution.admin.user.create"),
                "permissions"=>unserialize(AdminGroup::find(get_logged_in_user()->admin_group_id)->permissions),
                "all_groups"=>AdminGroup::where([
                    "deleted_at"=>null, 
                    "status"=>"1",
    ...
    function form(Request $req){
        if ($req->segment(3)) {
            if(AdminUser::findOrFail(Crypt::decrypt($req->segment(3)))->update([
                "name"=>$req->post("name"),
                "email"=>$req->post("email"),
                "mobile"=>$req->post("mobile"),
                "password"=>($req->post("password")),
                "admin_group_id"=>Crypt::decrypt($req->post("group_id")),
            ...
            if(AdminUser::create([
                ...
                "admin_group_id"=>Crypt::decrypt($req->post("group_id")),
                // "status"=>$req->post("status"),
                "institution_id"=>Crypt::decrypt(Session::get("institution_id"))
            ])){
