...
$(document).ready(function () {
  ...
  $('table.msc-smart-table').each(function () {
    ...
    $container.prepend('<input type="text" class="msc-smart-table-search" placeholder="Search..." autocomplete="off">');

    // Search functionality
    $container.find('.msc-smart-table-search').on('keyup', function () {
    ...
    $table.on('click', 'thead th', function () {
        let $th = $(this);
        let $rows = $table.find('tbody tr').not('.msc-action-row');
        let index = $th.index();
        ...
        }).forEach((row) => $table.find('tbody').append(row));
    });

    // Add 3-dot button in last column
    $table.find('tbody tr').each(function () {
        let $lastTd = $(this).find('td:last');
        $lastTd.data('action-html', $lastTd.html()); // Save original content
        $lastTd.html('<button class="msc-action-btn btn btn-warning"><i class="fas fa-ellipsis-h"></i> Actions</button>');
    });

    // Toggle expand on 3-dot button click
    $table.on('click', '.msc-action-btn', function (e) {
        ...
        if ($row.hasClass('msc-active')) {
            $table.find('.msc-action-row').slideUp(200, function () { $(this).remove(); });
   ...
   $(document).on("click","#deleteConfirmationModal #confirmDelete",function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(".MscDeleteRowBtn[href='"+$("#deleteConfirmationModal #confirmDelete").attr("data-rem-uri")+"']").closest("tr").remove();
            callBackStringToFunction(callbackStr);
        },
        error: (err)=>{
            try {
