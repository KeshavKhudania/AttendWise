...
<div style="margin-bottom: 16px;">
    <h3 style="font-weight: 800; font-size: 1.2rem; color: var(--text-main); margin-bottom: 4px;">Class Schedule</h3>
...
<div style="display: flex; gap: 8px; overflow-x: auto; padding-bottom: 10px; margin-bottom: 14px; scrollbar-width: none;">
    ...
    <button class="day-tab-btn {{ $isToday ? 'active' : '' }}" onclick="switchDay('{{ strtolower($day) }}')" id="tab-{{ strtolower($day) }}" style="background: {{ $isToday ? 'var(--text-main)' : 'transparent' }}; color: {{ $isToday ? 'var(--bg)' : 'var(--text-muted)' }}; border: 1px solid {{ $isToday ? 'transparent' : 'var(--border)' }}; padding: 8px 16px; border-radius: 14px; font-size: 0.82rem; font-weight: 700; white-space: nowrap; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.03);">
        {{ $day }}
        @if($isToday) <span style="font-size: 0.65rem; background: rgba(255,255,255,0.2); padding: 2px 6px; border-radius: 10px; margin-left: 4px;">Today</span> @endif
    </button>
    @endforeach
</div>
...
<div class="day-schedule-pane" id="pane-{{ $dayKey }}" style="display: {{ $isToday ? 'block' : 'none' }};">
    @if($daySchedules->isEmpty())
        <div class="glass-card" style="text-align: center; padding: 28px; color: var(--text-muted);">
            <i class="fa-regular fa-calendar-xmark" style="font-size: 2rem; color: var(--text-muted); margin-bottom: 8px;"></i>
            <div style="font-size: 0.9rem; font-weight: 700; color: var(--text-main);">No Classes Scheduled for {{ $day }}</div>
        </div>
    @else
        @foreach($daySchedules as $schedule)
        <div class="glass-card" style="padding: 16px; margin-bottom: 12px; border-left: 4px solid var(--text-main);">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <div>
                    <div style="font-size: 0.78rem; font-weight: 700; color: var(--text-main); margin-bottom: 4px;">
                    ...
                    <div style="font-weight: 800; font-size: 1.05rem; color: var(--text-main); margin-bottom: 4px;">
                ...
                <div style="background: #e0e7ff; border: 1px solid #c7d2fe; font-size: 0.72rem; font-weight: 700; color: #4f46e5; padding: 4px 10px; border-radius: 12px;">
                    Lecture
                </div>
            </div>
        </div>
...
<script>
    function switchDay(dayKey) {
        ...
        if (activeTab) {
            activeTab.style.background = 'var(--text-main)';
