...
<div style="margin-bottom: 16px;">
    <h3 style="font-weight: 800; font-size: 1.2rem; color: #fff; margin-bottom: 4px;">Class Schedule</h3>
...
<div style="display: flex; gap: 8px; overflow-x: auto; padding-bottom: 10px; margin-bottom: 14px; scrollbar-width: none;">
    ...
    <button class="day-tab-btn {{ $isToday ? 'active' : '' }}" onclick="switchDay('{{ strtolower($day) }}')" id="tab-{{ strtolower($day) }}" style="background: {{ $isToday ? 'var(--accent-gradient)' : 'rgba(26, 34, 52, 0.75)' }}; color: #fff; border: 1px solid var(--card-border); padding: 8px 16px; border-radius: 14px; font-size: 0.82rem; font-weight: 700; white-space: nowrap; cursor: pointer;">
        {{ $day }}
...
<div class="day-schedule-pane" id="pane-{{ $dayKey }}" style="display: {{ $isToday ? 'block' : 'none' }};">
    @if($daySchedules->isEmpty())
        <div class="glass-card" style="text-align: center; padding: 28px; color: var(--text-muted);">
            <i class="fa-regular fa-calendar-xmark" style="font-size: 2rem; color: #818cf8; margin-bottom: 8px;"></i>
            <div style="font-size: 0.9rem; font-weight: 600; color: #cbd5e1;">No Classes Scheduled for {{ $day }}</div>
        </div>
    @else
        ...
        <div class="glass-card" style="padding: 16px; margin-bottom: 12px; border-left: 4px solid var(--accent-primary);">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <div>
                    <div style="font-size: 0.78rem; font-weight: 700; color: #818cf8; margin-bottom: 4px;">
                ...
                <div style="background: rgba(99, 102, 241, 0.15); border: 1px solid rgba(99, 102, 241, 0.3); font-size: 0.72rem; font-weight: 700; color: #a5b4fc; padding: 4px 10px; border-radius: 12px;">
                    Lecture
                </div>
            </div>
        </div>
...
<script>
    function switchDay(dayKey) {
        ...
        document.querySelectorAll('.day-tab-btn').forEach(btn => {
            btn.style.background = 'rgba(26, 34, 52, 0.75)';
